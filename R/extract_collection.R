# CCHS Document Collection Extractor
# Extracts curated document collections from the CCHS catalog for RAG and other uses
...
#' Extract Document Collection from CCHS Catalog
#' 
...
#' @param catalog_file Path to CCHS catalog YAML file
#' @param source_dir Path to OSF documents directory  
#' @param languages Languages to include ("EN", "FR", or both)
#' @param years Year range to include (NULL for all years)
...
extract_collection <- function(target_categories = c("questionnaire", "data-dictionary", "derived-variables", "user-guide"),
                               output_dir = "rag-documents",
                               catalog_file = "data/catalog/cchs_catalog.yaml",
                               source_dir = "cchs-osf-docs",
                               languages = c("EN", "FR"),
                               years = NULL,
                               exclude_syntax = TRUE,
                               create_zip = TRUE,
  ...
  cat("=== CCHS Document Collection Extractor ===\n")
...
#' Filter catalog files by criteria
filter_catalog_files <- function(catalog, categories, languages, years, exclude_syntax = TRUE) {
  files <- catalog$files
  selected <- list()
  ...
  for (i in seq_along(files)) {
    ...
    # Check language
    if (!file_entry$language %in% languages) next
...
extract_files <- function(selected_files, source_dir, output_dir) {
  ...
  for (file_entry in selected_files) {
    # Construct source path using local_path from catalog
    source_path <- file.path(source_dir, file_entry$local_path)
    ...
    # Track the mapping for traceability with enhanced metadata
    file_mapping <- list(
      cchs_uid = file_entry$cchs_uid,
