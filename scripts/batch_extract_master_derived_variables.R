#!/usr/bin/env Rscript
# Batch extract structured data from master CCHS derived variable PDFs
#
# This script reads from the master PDF catalog and extracts
# English derived variable documents to YAML format.
#
# Usage:
#   Rscript batch_extract_master_derived_variables.R [base_dir]
#
...
cat("=== Batch Master Derived Variables Extraction ===\n\n")
...
# Filter to English derived variables files
dv_to_extract <- catalog[
  catalog$category == "derived-variables" &
  catalog$language == "EN",
]

cat("English derived-variables files:", nrow(dv_to_extract), "\n\n")
...
for (i in seq_len(nrow(dv_to_extract))) {
  src <- dv_to_extract[i, ]
  ...
  tryCatch({
    ...
    result <- parse_derived_variables(lines)
